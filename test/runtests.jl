...
@testset begin "integral"
    f(x) = cos(x)
    f(x, p; σ = 2, λ = 0) = (σ + cos(p*x))*exp(-abs(x)*λ)
    @test integral(f, Domain.Box1D(0,π/2)) ≈ 1
    @test integral(f, Domain.Box1D(0,π/2), 2; σ = 1) ≈ π/2
    @test f |> integral(Domain.Box1D(0,π/2), 2; σ = 1) ≈ π/2
    @test integral(x -> exp(-x), Domain.Box1D(0, Infinity(1))) ≈ 1
    @test integral((x,y) -> exp(-x-y), Domain.Box((0, 0), (Infinity(1), Infinity(1)))) ≈ 1
    @test integral((x; σ = 1) -> exp(-x/σ), Domain.Box1D((; σ=1) -> (0, Infinity(σ))); σ = 4) ≈ 4
    @test integral((x, a; σ = 1) -> a*exp(-x/σ), Domain.Box1D((a; σ=1) -> (0, Infinity(σ))), 4; σ = 4) ≈ 16
end
