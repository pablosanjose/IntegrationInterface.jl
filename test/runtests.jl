...
@testset begin "integral"
    ...
    @test Domain.is_obviously_empty(Domain.interval(2,2.0))
    @test Domain.is_obviously_empty(Domain.Box((1,1), (1,2)))
    @test Domain.is_obviously_empty(Domain.Simplex((1,1), (1,2), (1,1)))
    @test integral(Returns(im), Domain.interval(1,1); zerofastpath = true) === 0.0 + 0.0im
    @test integral(Returns(0f0), Domain.Box((1,1f0+0im), (1,2)); zerofastpath = true) === 0.0f0 + 0.0f0im
    @test integral(Returns(0), Domain.Simplex((1,1f0), (1,2), (1,1)); zerofastpath = true) === 0.0f0
end
